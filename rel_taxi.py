...
with st.expander("📖 Xem Luật Chơi Chi Tiết (Taxi MDP)", expanded=False):
    ...
    * **Phần thưởng (Rewards):**
        ...
        * Đón/Trả khách sai vị trí: Phạt nặng **-10 điểm**.
        * Trả khách đúng đích đến: Thưởng lớn **+20 điểm** và kết thúc game.
                    ...
                    for act_idx, act in enumerate(actions):
                        if act_idx < 4:
                            ...
                            if r == 0 and c == 0 and has_pass == 0:
                                v_acts.append(-1 + gamma * V[r, c, 1])
                            else:
                                v_acts.append(-10 + gamma * V[r, c, has_pass])
                        elif act == '🏁 Trả Khách':
                            if r == 4 and c == 4 and has_pass == 1:
                                v_acts.append(20 + 0)
                            else:
...
with tab1:
    ...
    # === BỔ SUNG: KHU VỰC THIẾT LẬP VỊ TRÍ BAN ĐẦU ===
    st.markdown("### 📍 Thay đổi / Thiết lập vị trí xuất phát")
    col_sel1, col_sel2, col_sel3 = st.columns([1, 1, 1.5])
    with col_sel1:
        start_r = st.selectbox("Chọn Hàng (0-4):", range(5), index=int(st.session_state.tx_r), key="tx_row")
    with col_sel2:
        start_c = st.selectbox("Chọn Cột (0-4):", range(5), index=int(st.session_state.tx_c), key="tx_col")
    with col_sel3:
        st.write("") 
        st.write("")
        if st.button("🚀 Đặt Taxi vào vị trí này", use_container_width=True, type="secondary"):
            if (start_r, start_c) in BUILDINGS:
                st.error("❌ Lỗi: Bạn không thể đặt xe lên nóc chung cư!")
            else:
                st.session_state.tx_r = start_r
                st.session_state.tx_c = start_c
                st.session_state.tx_has_pass = 0
                st.session_state.tx_score = 0
                st.session_state.tx_logs = [f"🚀 Đã điều xe tới: Hàng {start_r}, Cột {start_c}! Bắt đầu làm việc."]
    ...
    def do_action(action, dr=0, dc=0):
        ...
        if action in ['⬆️ Lên', '⬇️ Xuống', '⬅️ Trái', '➡️ Phải']:
            ...
            if r == 4 and c == 4 and hp == 1:
                ...
                log = f"🎉 TRẢ KHÁCH THÀNH CÔNG! Thưởng +20. Tổng: {st.session_state.tx_score} điểm."
                st.balloons()
            else:
                st.session_state.tx_score -= 10
    ...
    with col_ctrl:
        ...
        if st.button("🔄 Reset Về Mặc Định", use_container_width=True): reset_game(); st.rerun()
